...
public class IBEET_DBA {
	...
	public List<Map<String, Object>> setup() {
		...
		Element h_1 = pairing.getG1().newRandomElement().getImmutable();
		Element h_2 = pairing.getG1().newRandomElement().getImmutable();
		Element h_3 = pairing.getG1().newRandomElement().getImmutable();
	...
	public Map<String, Object> keygen(Map<String, Object> pk, Map<String, Object> msk, String ID) {
		...
		Element g_ID_2 = Utils.bytes2element(byte_F_2_ID, "G1").getImmutable();

		// Use F_3 to hash ID
		byte[] byte_F_3_ID = F_3.hash(ID.getBytes());
		Element g_ID_3 = Utils.bytes2element(byte_F_3_ID, "G1").getImmutable();
	...
	public Map<String, Object> encrypt(Map<String, Object> pk, String ID, byte[] m, String[] T) {
		...
		Element F_2_ID = Utils.bytes2element(byte_F_2, "G1").getImmutable();
		...
		// Compute C_2
		Element h_m_r = Utils.bytes2element(byte_m, "G1");
		h_m_r.powZn(r);
		...
		Element F_3_ID = Utils.bytes2element(byte_F_3, "G1").getImmutable();
	...
	public byte[] decrypt(Map<String, Object> pk, Map<String, Object> sk, Map<String, Object> ct) {
		...
		Element A = pairing.getG1().newElementFromBytes(byte_A).getImmutable();

		// Check equation
		Element h_m = Utils.bytes2element(m, "G1");
	...
	public Map<String, Object> auth(Map<String, Object> pk, Map<String, Object> sk, String[] T, int type) {
		...
		Element pre_compute_item = (h_0.powZn(pairing.getZr().newElementFromBytes(ID.getBytes())))
				.mul(h_1.powZn(pairing.getZr().newElementFromBytes(T[0].getBytes()))).mul(g_2);
		Element T_0 = null;
		Element T_1 = null;
		Element T_2 = null;
		Element T_3 = null;
		...
		if (type == 1) {
			...
			T_0 = K_1.mul(
					(pre_compute_item.mul(h_2.powZn(pairing.getZr().newElementFromBytes(T[1].getBytes())))).powZn(r))
					.getImmutable();
			T_1 = g.powZn(r).getImmutable();
			T_2 = h_2.powZn(r).getImmutable();
			T_3 = h_3.powZn(r).getImmutable();
			...
			T_0 = K_1
					.mul((pre_compute_item.mul(h_2.powZn(pairing.getZr().newElementFromBytes(T[1].getBytes())))
							.mul(h_3.powZn(pairing.getZr().newElementFromBytes(T[2].getBytes())))).powZn(r))
					.getImmutable();
			T_1 = g.powZn(r).getImmutable();
			T_2 = h_2.powZn(r).getImmutable();
			T_3 = h_3.powZn(r).getImmutable();
	...
	public Object test(Map<String, Object> pk, Map<String, Object> ct_1, Map<String, Object> td_1,
			Map<String, Object> ct_2, Map<String, Object> td_2) {

		long startTime = System.nanoTime();

		if (ct_1 == null || !(((String) ct_1.get("type")).equals("ct")) || ct_2 == null
				|| !(((String) ct_2.get("type")).equals("ct"))) {
		...
		Element delta_1 = IBEET_DBA.compute(pk, ct_1, td_1, pairing);
		Element delta_2 = IBEET_DBA.compute(pk, ct_2, td_2, pairing);
	...
	private static Element compute(Map<String, Object> pk, Map<String, Object> ct, Map<String, Object> td,
		...
		Element sum = null;
		if (type == 1) {
			sum = (pairing.pairing(C_1, T_0.mul(T_2.powZn(T_elem[1]).mul(T_3.powZn(T_elem[2])))))
					.div(pairing.pairing(C_3, T_1)).getImmutable();
		} else if (type == 2) {
			sum = (pairing.pairing(C_1, T_0.mul(T_3.powZn(T_elem[2])))).div(pairing.pairing(C_3, T_1)).getImmutable();
		} else {
			sum = (pairing.pairing(C_1, T_0)).div(pairing.pairing(C_3, T_1)).getImmutable();
		}

		byte[] cita = H_1.hash(sum.toBytes());
		byte[] delta = Utils.byte_array_Xor(C_2, cita);

		Element res = Utils.bytes2element(delta, "G1").getImmutable();
