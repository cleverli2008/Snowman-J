...
public class IBEET_DBA {
	...
	private Pairing pairing = PairingFactory.getPairing("curves/a.properties");
...
//		long startTime = System.nanoTime();
		...
		Element g_ID_2 = Function.bytes2element(byte_F_2_ID, "G1", pairing).getImmutable();
		
		//Use F_3 to hash ID
		byte[] byte_F_3_ID = F_3.hash(ID.getBytes());
		Element g_ID_3 = Function.bytes2element(byte_F_3_ID, "G1", pairing).getImmutable();
...
//		long startTime = System.nanoTime();
		...
		Element F_2_ID = Function.bytes2element(byte_F_2, "G1", pairing).getImmutable();
		...
		byte[] byte_m = Function.byte_array_Xor(Function.byte_array_Xor(zero, m),zero);
		
		//Compute C_2
		Element h_m_r = Function.bytes2element(byte_m, "G1", pairing);
		h_m_r.powZn(r);
		byte[] C_2 = Function.byte_array_Xor(byte_e_gg, h_m_r.toBytes());
		...
		Element F_3_ID = Function.bytes2element(byte_F_3, "G1", pairing).getImmutable();
		...
		byte[] C_4 = Function.byte_array_Xor(hash_val, m);
...
//		long startTime = System.nanoTime();
		...
		byte[] m = Function.byte_array_Xor(C_4, hash_val);
				
		//Compute A
		Element e_gg_F_2 = pairing.pairing(C_1, K_1).getImmutable();
		byte[] byte_e_gg = H_1.hash(e_gg_F_2.toBytes());
		
		byte[] byte_A = Function.byte_array_Xor(C_2, byte_e_gg);
		
		Element A = pairing.getG1().newElementFromBytes(byte_A).getImmutable();
		
		//Check equation 
		Element h_m = Function.bytes2element(m, "G1", pairing);
...
//			System.out.print(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
			...
			System.out.print("Test: Not matching!");
			return false;
		}
	}
	...
	private static Element compute(Map<String, Object> pk, Map<String, Object> ct, Map<String, Object> td, Pairing pairing) {
		...
		Element sum = null;
		if(type == 1){
			sum = (pairing.pairing(C_1,T_0.mul(T_2.powZn(T_elem[1]).mul(T_3.powZn(T_elem[2]))))).div(pairing.pairing(C_3,T_1)).getImmutable();
		}
		else if (type == 2){
		...
		byte[] delta = Function.byte_array_Xor(C_2, cita);
		
		//Element res = pairing.getG1().newElementFromBytes(delta).getImmutable();
		Element res = Function.bytes2element(delta, "G1", pairing).getImmutable();
