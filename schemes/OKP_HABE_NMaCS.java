...
public class OKP_HABE_NMaCS {
		...
		public Map[] setup(int q) {
			...
			Element e_gg_alpha = pairing.pairing(g, g).powZn(alpha).getImmutable();
...
//			System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");		
		...
		private Map<String, Object> authkeygen(Map<String, Object> pk, Map<String, Object> msk, String[] id_vector){
			...
			Element K_1 = g.powZn(r).getImmutable();
			ak.put("K_0", K_0);
			ak.put("K_1", K_1);
			ak.put("U_0", U_0);
...
//			System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");	
	...
	private Map<String, Object> authdelegate(Map<String, Object> pk, Map<String, Object> ak, String id){
			...
			K_1 = K_1.mul(g.powZn(r_1)).getImmutable();
			ak_new.put("K_1",K_1);
			...
			delta = (delta.mul(g_1.mul(h_j.powZn(pairing.getZr().newElementFromBytes(id.getBytes()))))).powZn(r_1).getImmutable();
...
//			System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
	...
	public Map<String, Object> encrypt(Map<String, Object> pk, String[] id_vector, Attribute[] set_s, String message) {
		...
		Element[] a = (Element[]) addArrays(coeff, zero);
...
//		System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
		...
		public Map<String, Object>[] userkeygen(Map<String, Object> pk, Map<String, Object> ak, String str_policy) {
			...
			for(int i = 0; i < id_vector.length; i++) {
				Element h_i= ((Element)pk.get("h_" + (i + 1))).duplicate().getImmutable();
				pre_term = pre_term.mul(h_i.powZn(pairing.getZr().newElementFromBytes(id_vector[i].getBytes()))).getImmutable();
			}
...
//			y[0] = alpha;
			...
			for (int tau = 0; tau < matrix.length; tau++) {
				...
				String attr_val = this.splitAttribute(attr, ":")[1];
				...
				if(attr_val.charAt(0)!='-'){
					Element K_tau_0_1 = g.powZn(share_term.div(o)).mul(pre_term_two_non.powZn(M_tau_0.div(o))).mul(pre_term_non.powZn(r_tau)).getImmutable();
					Element K_tau_1_1 = K_1.powZn(M_tau_0.div(o)).mul(g.powZn(r_tau)).getImmutable();
					sk.put("K_"+ tau + "_0_1", K_tau_0_1);
					sk.put("K_"+ tau + "_1_1", K_tau_1_1);
				}
				else {
					Element K_tau_0_2 = g.powZn(share_term.div(o)).mul(pre_term_two_neg.powZn(M_tau_0.div(o))).mul(pre_term_neg.powZn(r_tau)).getImmutable();
					Element K_tau_1_2 = K_1.powZn(M_tau_0.div(o)).mul(g.powZn(r_tau)).getImmutable();
				...
				for(int i = 2; i <= q; i++) {
					...
					if(attr_val.charAt(0)!='-'){
						...
						Element K_tau_i_1 = ((U_1.powZn((x_tau.powZn(exp)).negate()).mul(U_i)).powZn(M_tau_0.div(o))).mul((u_1.powZn((x_tau.powZn(exp)).negate()).mul(u_i)).powZn(r_tau)).getImmutable();
						...
						Element K_tau_i_2 = ((V_1.powZn((x_tau.powZn(exp)).negate()).mul(V_i)).powZn(M_tau_0.div(o))).mul((v_1.powZn((x_tau.powZn(exp)).negate()).mul(v_i)).powZn(r_tau)).getImmutable();
...
//		System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
		...
		Element[] values = new Element[set_size];
		for(int i = 0; i < set_size; i++) {
			values[i] = pairing.getZr().newElementFromBytes(attributes[i].getAttrValue().getBytes()).negate().getImmutable();
		}
		...
		for (int i = 0; i < q-set_size; i++) {
		 ...
		 for (int i = 0; i < attrsetI.length; i++) {
			 ...
			 if(attr_val.charAt(0)!='-'){
				 ...
				 B = B.mul((pairing.pairing(C_0, K_tilde_i_0_1).div(pairing.pairing(C_1.mul(C_3), K_i_1_1))).powZn(omega[i])).getImmutable();
			 }
			 else {
				 ...
				 T_i = (pairing.pairing(C_0,K_tilde_i_2).div(pairing.pairing(C_2, K_i_1_2))).powZn(pairing.getZr().newOneElement().div(sum).negate()).getImmutable();
				 B = B.mul((pairing.pairing(C_0,K_i_0_2).div(pairing.pairing(C_3,K_i_1_2).mul(T_i))).powZn(omega[i])).getImmutable();
			 }
		  }
...
//		System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");		
	...
	public Element decrypt(Map<String, Object> it, Map<String, Object> dk){
		...
		long endTime = System.nanoTime();
//		System.out.println(String.format("%.4f", (float)((endTime - startTime) / 1_000_000.0000)));	
		return m;
		}
	...
	public static String toString(Map<String, Object> map){
		...
		public String[] splitAttribute(String attr, String symbol) {     
		...
		public static void main(String[] args) throws Exception {
			...
			String[] id_vector = {"I_1"};
			Map ak = scheme.authkeygen(pk, msk, id_vector);
			System.out.println(KP_HABE_NMaCS.toString(ak) + "\n");
			
			int i = 3;
			
			if(i == 0) {
			String[] id_vector_rec = {"I_1"};
			String message = "This a test!";
			Attribute[] attributes = new Attribute[3];
			attributes[0] = new Attribute("school", "pku");
			attributes[1] = new Attribute("academy", "computer");
			attributes[2] = new Attribute("id", "1701110686");
			Map ct = scheme.encrypt(pk, id_vector_rec, attributes, message);
			System.out.println(KP_HABE_NMaCS.toString(ct) + "\n");
			
			String policy  = "school:pku and academy:computer and id:1701110686";
			Map[] key = scheme.userkeygen(pk, ak, policy);
			Map sk = key[0];
			Map dk = key[1];
			System.out.println(KP_HABE_NMaCS.toString(sk) + "\n");
			...
			System.out.println(KP_HABE_NMaCS.toString(ak_new) + "\n");
			
			String[] id_vector_rec = {"I_1","I_3"};
			String message = "This a test!";
			...
			Map[] key = scheme.userkeygen(pk, ak_new, policy);
			Map sk = key[0];
			Map dk = key[1];
			System.out.println(KP_HABE_NMaCS.toString(sk) + "\n");
			...
			System.out.println("m:" + m + "\n");
				...
				String id_2 = "I_3";
				Map ak_new_2 = scheme.authdelegate(pk, ak_new, id_2);
				System.out.println(KP_HABE_NMaCS.toString(ak_new_2) + "\n");
				
				String[] id_vector_rec = {"I_1","I_2","I_3"};
				String message = "This a test!";
				Attribute[] attributes = new Attribute[2];
				attributes[0] = new Attribute("school", "pku");
				attributes[1] = new Attribute("academy", "computer");
				...
				//String[] id_vector_send = {"I_1","I_2"};
				String policy  = "school:pku and academy:computer";
				Map[] key = scheme.userkeygen(pk, ak_new_2, policy);
				Map sk = key[0];
				Map dk = key[1];
				System.out.println(KP_HABE_NMaCS.toString(sk) + "\n");
				...
				String policy  = "academy:-math and id:-1701110687";
				Map[] key = scheme.userkeygen(pk, ak_new, policy);
				Map sk = key[0];
				Map dk = key[1];
