...
//			System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
	...
	public Map authdelegate(Map<String, Object> pk, Map<String, Object> ak, String id){
		...
		public Map<String, Object> userkeygen(Map<String, Object> pk, Map<String, Object> ak, String str_policy) {
			...
			for (int tau = 0; tau < matrix.length; tau++) {
				...
				if(attr_val.charAt(0)!='-'){
					Element K_tau_0_1 = g.powZn(share_term).mul(pre_term_two_non.powZn(M_tau_0)).mul(pre_term_non.powZn(r_tau)).getImmutable();
					Element K_tau_1_1 = K_1.powZn(M_tau_0).mul(g.powZn(r_tau)).getImmutable();
					sk.put("K_"+ tau + "_0_1", K_tau_0_1);
					sk.put("K_"+ tau + "_1_1", K_tau_1_1);
				}
				else {
					Element K_tau_0_2 = g.powZn(share_term).mul(pre_term_two_neg.powZn(M_tau_0)).mul(pre_term_neg.powZn(r_tau)).getImmutable();
					Element K_tau_1_2 = K_1.powZn(M_tau_0).mul(g.powZn(r_tau)).getImmutable();
				...
				for(int i = 2; i <= q; i++) {
					...
					if(attr_val.charAt(0)!='-'){
						...
						Element K_tau_i_1 = ((U_1.powZn((x_tau.powZn(exp)).negate()).mul(U_i)).powZn(M_tau_0)).mul((u_1.powZn((x_tau.powZn(exp)).negate()).mul(u_i)).powZn(r_tau)).getImmutable();
						...
						Element K_tau_i_2 = ((V_1.powZn((x_tau.powZn(exp)).negate()).mul(V_i)).powZn(M_tau_0)).mul((v_1.powZn((x_tau.powZn(exp)).negate()).mul(v_i)).powZn(r_tau)).getImmutable();
...
//		System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
		...
		public Element decrypt(Map<String, Object> pk, Map<String, Object> ct, Map<String, Object> sk) {
...
//			System.out.println(String.format("%.2f", (float)((endTime - startTime) / 1_000_000.0000))+ " ");
		...
		public static void main(String[] args) throws Exception {
			...
			Map sk = scheme.userkeygen(pk, ak_new, policy);
			System.out.println(KP_HABE_NMaCS.toString(sk) + "\n");
			...
			System.out.println("m:" + m + "\n");
				...
				String policy  = "school:pku and academy:computer";
				Map sk = scheme.userkeygen(pk, ak_new_2, policy);
				System.out.println(KP_HABE_NMaCS.toString(sk) + "\n");
