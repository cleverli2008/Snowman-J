...
/**
 * @ClassName: XSLD20
 * @Description:(Implementation of "Identity-Based Chameleon Hash without Random
 *               Oracles and Application in the Mobile Internet")
 * @author:cleverli2008
 * @date:2021/9/30
 */
...
public class XSLD20 {
	...
	public Element[] hash(Map<String, Object> pk, int[] ID, byte[] m) {
		...
		Element h = pairing.pairing(g_1, g_2).powZn(ele_m)
				.mul(pairing.pairing(r_1, g).div(pairing.pairing(r_2, prod_u_i))).getImmutable();
	...
	public Boolean verfiy(Map<String, Object> pk, Element[] r_new, byte[] m_prime, int[] ID, Element h) {
		...
		Element h_prime = pairing.pairing(g_1, g_2).powZn(ele_m_prime)
				.mul(pairing.pairing(r_1_prime, g).div(pairing.pairing(r_2_prime, prod_u_i))).getImmutable();

		// If the equation (1) holds, the algorithm returns true; otherwise, it returns
		// false
		if (h_prime.equals(h))
			return true;
		else
